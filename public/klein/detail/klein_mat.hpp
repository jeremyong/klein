...
namespace kln
{
    ...
    inline void KLN_VEC_CALL mat4x4_12(__m128 const& b, __m128 const* c, __m128* out) noexcept
    {
        ...
        //  2a1(b1 b3 - b0 b2) +
        //  2a2(b0 b1 + b2 b3) +
        //  a3(b0^2 + b3^2 - b1^2 - b2^2)) e032 // x-coordinate
        //
        ...
        //  2a2(b1 b2 - b0 b3) +
        //  2a3(b0 b2 + b1 b3) +
        //  a1(b0^2 + b1^2 - b2^2 - b3^2)) e021 + // z-coordinate
        //
        ...
        float buf[4];
        _mm_storeu_ps(buf, b);
        float b0_2 = buf[0]*buf[0];
        float b1_2 = buf[1]*buf[1];
        float b2_2 = buf[2]*buf[2];
        float b3_2 = buf[3]*buf[3];

        // The first column of the matrix we need to produce contains the scale
        // factors of the x-coordinate (a3). This can be read off as:
        //
        // b0^2 + b3^2 - b1^2 - b2^2
        // 2(-b1 b0 + b2 b3)
        // 2(b2 b0 + b1 b3)
        // 0
        __m128& c0 = out[0];
        c0 = _mm_mul_ps(b, KLN_SWIZZLE(b, 0, 0, 0, 0));
        __m128 tmp = _mm_mul_ps(KLN_SWIZZLE(b, 0, 1, 2, 3), KLN_SWIZZLE(b, 0, 3, 3, 3));
        tmp = _mm_mul_ps(_mm_set_ps(0.f, 1.f, -1.f, 1.f), tmp);
        c0 = _mm_mul_ps(_mm_set_ps(0.f, 2.f, -2.f, 1.f), _mm_add_ps(c0, tmp));
        c0 = _mm_sub_ps(c0, _mm_set_ss(b1_2 + b2_2));
        ...
        // 0
        __m128& c1 = out[1];
        c1 = _mm_mul_ps(b, KLN_SWIZZLE(b, 0, 1, 1, 1));
        tmp = _mm_mul_ps(KLN_SWIZZLE(b, 0, 0, 3, 2), KLN_SWIZZLE(b, 0, 3, 3, 3));
        tmp = _mm_mul_ps(_mm_set_ps(0.f, -1.f, 1.f, 1.f), tmp);
        ...
        // 0
        __m128& c2 = out[2];
        c2         = _mm_mul_ps(_mm_set_ps(0.f, -1.f, 1.f, -1.f),
                        _mm_mul_ps(b, KLN_SWIZZLE(b, 0, 2, 2, 2)));
        c2         = _mm_add_ps(
            c2,
            _mm_mul_ps(KLN_SWIZZLE(b, 0, 0, 0, 1), KLN_SWIZZLE(b, 0, 0, 3, 3)));
        c2 = _mm_mul_ps(c2, _mm_set_ps(0.f, 1.f, 2.f, 2.f));
        c2 = _mm_add_ps(c2, _mm_set_ps(0.f, b1_2 - b3_2, 0.f, 0.f));

        // And finally w (a0)
        //
        // 2(b2 c3 - b0 c1 - b1 c2 - b3 c0) 
        // 2(b1 c1 - b0 c2 - b3 c3 - b2 c0)
        // 2(b3 c2 - b0 c3 - b2 c1 - b1 c0)
        // b0^2 + b1^2 + b2^2 + b3^2
        __m128& c3 = out[3];
        if constexpr (Translate)
        {
            c3 = _mm_mul_ps(
                KLN_SWIZZLE(b, 1, 0, 0, 0), KLN_SWIZZLE(*c, 0, 3, 2, 1));
            c3  = _mm_add_ps(c3,
                            _mm_mul_ps(KLN_SWIZZLE(b, 0, 2, 3, 1),
                                       KLN_SWIZZLE(*c, 0, 1, 3, 2)));
            c3  = _mm_add_ps(c3,
                            _mm_mul_ps(KLN_SWIZZLE(b, 3, 1, 2, 3),
                                       KLN_SWIZZLE(*c, 0, 0, 0, 0)));
            tmp = _mm_mul_ps(
                KLN_SWIZZLE(b, 0, 3, 1, 2), KLN_SWIZZLE(*c, 0, 2, 1, 3));
            c3 = _mm_mul_ps(_mm_set_ps(0.f, 2.f, 2.f, 2.f), _mm_sub_ps(tmp, c3));
            c3 = _mm_add_ps(
                _mm_set_ps(b0_2 + b1_2 + b2_2 + b3_2, 0.f, 0.f, 0.f), c3);
        }
        else
        {
